...
namespace Cms.BLL
{
	...
	{
        private readonly SQLServerDAL.C_article_category dal=new SQLServerDAL.C_article_category();
        ...
        {
            return dal.GetList(parent_id, channel_id, Where);
        }
        #endregion
		/// <summary>
		/// 得到最大ID
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Cms.Model.C_article_category GetModelByCache(int id)
		{
			
			string CacheKey = "C_article_categoryModel-" + id;
			...
			return (Cms.Model.C_article_category)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Cms.Model.C_article_category> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Cms.Model.C_article_category> DataTableToList(DataTable dt)
		{
			List<Cms.Model.C_article_category> modelList = new List<Cms.Model.C_article_category>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Cms.Model.C_article_category model;
