...
namespace Cms.BLL
{
	...
	{
        private readonly SQLServerDAL.C_admin dal=new SQLServerDAL.C_admin();
		public C_admin()
		{}
		#region  BasicMethod
        ...
        /// 是否存在该用户
        /// </summary>
        public bool ExistsUser(string userName, string userPwd)
        {
            return dal.ExistsUser(userName, userPwd);
        }
		/// <summary>
		/// 是否存在该记录
		/// </summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Cms.Model.C_admin GetModelByCache(int id)
		{
			
			string CacheKey = "C_adminModel-" + id;
			...
			return (Cms.Model.C_admin)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Cms.Model.C_admin> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Cms.Model.C_admin> DataTableToList(DataTable dt)
		{
			List<Cms.Model.C_admin> modelList = new List<Cms.Model.C_admin>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Cms.Model.C_admin model;
